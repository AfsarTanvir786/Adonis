...
export class AuthUtils {
  ...
  static async ensureCompanyExists(user: any, noteService: any, response: HttpContext['response']) {
  ...
  static async ensureWorkspaceAccess(
    ...
    if (workspaceIds.length === 0 || !workspaceIds.includes(payloadWorkspaceId)) {
