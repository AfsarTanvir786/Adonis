...
export default class NoteVoteSeeder extends BaseSeeder {
  async run() {
    ...
    const publicNotes = await Note.query().where('type', 'public').where('is_draft', false)
    ...
    for (const note of publicNotes) {
      ...
      const voterCount = Math.floor(Math.random() * Math.min(10, eligibleVoters.length)) + 1
      ...
      await VoteCount.updateOrCreate(
        { noteId: note.id },
        {
          noteId: note.id,
          upVoteCount: upVotes,
      ...
      console.log(`Note "${note.title}": ${upVotes} upvotes, ${downVotes} downvotes`)
    }
